...
func TestPeriodicParse(t *testing.T) {
	...
	for _, testCase := range tests {
		...
		t.Run(fmt.Sprintf("%v", testCase), func(t *testing.T) {
			...
			_, err := operator.ParsePeriodic(testCase.start, testCase.duration)
			if err != nil && testCase.err == false {
				t.Fatalf("Unexpected error: %v", err)
			}

			if err == nil && testCase.err == true {
				t.Fatalf("Expected error and did not get it")
			}
		})
	}
}
...
func TestToStart(t *testing.T) {
	...
	for _, testCase := range tests {
		...
		t.Run(testCase.time, func(t *testing.T) {
			...
			periodic, err := operator.ParsePeriodic(testCase.start, testCase.duration)
			if err != nil {
				t.Fatalf("Periodic parse failed: %v", err)
			}

			ref := mustParseTime(testCase.time)
			if dts := periodic.DurationToStart(ref); dts != testCase.toStart {
				t.Fatalf("Got %v, want %v", dts, testCase.toStart)
			}
		})
	}
}
